...
class HealthMonitor:
    ...
    def record_api_response(self, endpoint: str, response_time_ms: float, success: bool = True):
        with self.lock:
    ...
    def record_trade_execution(self, symbol: str, execution_time_ms: float):
        with self.lock:
    ...
    def record_wallet_balance(self, balance: float):
        with self.lock:
            ...
            if self.last_balance is not None:
                ...
                drop_pct = (self.last_balance - balance) / self.last_balance if self.last_balance > 0 else 0
                if drop_pct > 0.10:
                    msg = (
                    ...
                    logger.warning(f"Wallet balance dropped unexpectedly: {drop_pct:.1%}")
    ...
    def _generate_hourly_report(self, now_timestamp: float):
        uptime_hrs = (now_timestamp - self.start_time) / 3600
        
        avg_api_time = (sum(self.api_response_times) / len(self.api_response_times)) if self.api_response_times else 0.0
        avg_trade_time = (sum(self.trade_execution_times) / len(self.trade_execution_times)) if self.trade_execution_times else 0.0
