...
class MicrostructureAnalyzer:
    ...
    def __init__(self, rpc_url: str = "https://quote-api.jup.ag/v6"):
    ...
    def print_analysis(self, results: Dict):
        ...
        print(f"--- Microstructure Analysis: {results['token_pair']} ---")
        print(f"Recommendation: {results['execution_recommendation']}")
        print(f"Optimal Size (<= 1% impact): ${results['optimal_size_usd']}")
        print("\nPrice Impact Curve:")
        print(f"{'Size (USD)':<15} | {'Impact (%)':<15} | {'Hops':<10}")
        print("-" * 45)
        
        for curve in results["curves"]:
            print(f"${curve['size_usd']:<14} | {curve['price_impact_pct']:<14.4f} | {curve['route_plan']:<10}")
        print("-" * 45)
