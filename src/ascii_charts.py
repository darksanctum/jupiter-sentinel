...
def render_equity_curve(
    data: List[float], width: int = 60, height: int = 10, title: str = "Equity Curve"
) -> str:
    """
    ...
    # Build output
    out = [
        f"{Colors.BOLD}{Colors.CYAN}╭── {title} {'─' * max(0, width - len(title) - 1)}{Colors.RESET}"
    ]
    for idx, row in enumerate(grid):
        y_val = (
            max_val - (idx / (height - 1)) * (max_val - min_val)
            if height > 1
            else max_val
        )
        label = f"{y_val:8.2f} │"
...
def render_candlesticks(
    ...
    out = [
        f"{Colors.BOLD}{Colors.YELLOW}╭── {title} {'─' * max(0, width - len(title) - 1)}{Colors.RESET}"
    ]
    for idx, row in enumerate(grid):
        y_val = (
            max_val - (idx / (height - 1)) * (max_val - min_val)
        ...
        label = f"{y_val:8.2f} │"
        colored_row = "".join(
            colors_grid[idx][i] + char + Colors.RESET if char != " " else " "
...
def render_volume_bars(
    volumes: List[float], width: int = 60, height: int = 5, title: str = "Volume"
) -> str:
    """
    ...
    if len(volumes) > width:
        ...
        downsampled = [
            max(volumes[int(i * step) : int((i + 1) * step)]) for i in range(width)
        ]
    else:
        downsampled = volumes
        width = len(volumes)
    ...
    out = [
        f"{Colors.BOLD}{Colors.BLUE}╭── {title} {'─' * max(0, width - len(title) - 1)}{Colors.RESET}"
    ]
...
def render_bollinger_bands(
    ...
    out = [
        f"{Colors.BOLD}{Colors.WHITE}╭── {title} {'─' * max(0, width - len(title) - 1)}{Colors.RESET}"
    ]
    for idx, row in enumerate(grid):
        y_val = (
            max_val - (idx / (height - 1)) * (max_val - min_val)
