...
def analyze_multiple_timeframes() -> Dict[str, any]:
    ...
    normalized_score = combined_score / max_possible_score if max_possible_score > 0 else 0
...
if __name__ == "__main__":
    ...
    for tf, sig in result['signals'].items():
        print(f"  {tf:>5}: {sig.upper()} (Weight: {TIMEFRAMES[tf]})")
    print(f"Combined Score: {result['combined_score']} (out of +/-{result['max_possible_score']})")
