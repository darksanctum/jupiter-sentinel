    ...
    def scan_pair(
        ...
        for amount in amounts:
            try:
                ...
                resp = request_json(
                    req, timeout=10, describe=f"Route arbitrage quote {pair_name}"
                )
        ...
        for i, q1 in enumerate(quotes):
            for q2 in quotes[i + 1 :]:
                ...
                spread = abs(q1["price_per_unit"] - q2["price_per_unit"]) / min(
                    q1["price_per_unit"], q2["price_per_unit"]
                )
...
if __name__ == "__main__":
    ...
    if opps:
        logging.debug("%s", f"Found {len(opps)} arbitrage opportunities:")
        for o in opps:
            logging.debug("%s", f"  {o.pair}: {o.spread_pct:.2f}% spread")
            logging.debug("%s", f"    Buy via: {o.buy_route}")
            logging.debug("%s", f"    Sell via: {o.sell_route}")
    else:
        logging.debug("%s", "No arbitrage opportunities found (market is efficient)")
