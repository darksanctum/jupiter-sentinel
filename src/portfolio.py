...
class PortfolioManager:
    ...
    def __init__(self, risk_manager: RiskManager, total_capital_usd: float):
        self.risk_manager = risk_manager
        self.total_capital_usd = total_capital_usd
        self.dca_bots: Dict[str, DCABot] = {}
        self.dca_schedules: Dict[str, dict] = {}
        
    def allocate_capital(self, assets: List[str], strategy: str = "equal") -> Dict[str, float]:
        """
    ...
    def schedule_dca(
        ...
        """
        bot = DCABot(amount_per_buy_sol=amount_per_buy_sol, interval_seconds=interval_seconds)
        ...
        print(f"[DCA SCHEDULED] {pair} | {total_buys} buys of {amount_per_buy_sol} SOL every {interval_seconds}s")
        
    def execute_dca_step(self, pair: str, input_mint: str, output_mint: str) -> Optional[DCAState]:
        """
        ...
        if current_time >= schedule["next_buy_time"] and schedule["buys_executed"] < schedule["total_buys"]:
            ...
            if schedule["buys_executed"] >= schedule["total_buys"]:
                ...
                print(f"[DCA COMPLETED] {pair} DCA schedule finished.")
                
            return state
        return None

    def check_profit_locks(self, lock_threshold_pct: float = 0.10, lock_amount_pct: float = 0.50) -> List[dict]:
        """
        ...
        for pos in self.risk_manager.positions:
            ...
            if pnl_pct >= lock_threshold_pct:
                ...
                print(f"[PROFIT LOCK] {pos.pair} | Locked {locked_amount:.6f} SOL at {pnl_pct*100:+.2f}% profit. Stop-loss moved to breakeven.")
