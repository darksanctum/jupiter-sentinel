...
class PredictionMarketTracker:
    ...
    def _initialize_default_markets(self) -> Any:
    ...
    async def process_jupiter_signal(
        self, token_symbol: str, current_price: float, volatility: float
    ) -> Any:
        """
        ...
        for market_id, market_data in self.active_markets.items():
            if market_data["token"] == token_symbol:
                now = datetime.utcnow()
                time_to_res = (
                    market_data["resolution_time"] - now
                ).total_seconds() / 86400.0  # in days
                ...
                logger.debug(
                    f"Updated market {market_id}: YES={market_data['current_odds_yes']}, NO={market_data['current_odds_no']}"
                )
