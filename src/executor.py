...
class TradeExecutor:
    ...
    def _requires_position_limit(self, input_mint: str, output_mint: str) -> bool:
        """Function docstring."""
        return input_mint in {SOL_MINT, USDC_MINT} and output_mint not in {
            SOL_MINT,
            USDC_MINT,
        }
    ...
    def _sol_price_usd(self) -> float:
        ...
        if not sol_quote:
            raise RuntimeError(
                "Could not determine SOL price for hard position-limit enforcement"
            )
    ...
    def _input_notional_usd(self, input_mint: str, amount: int) -> float:
        ...
        raise RuntimeError(
            f"Unsupported mint for hard position-limit enforcement: {input_mint}"
        )

    def _enforce_hard_position_limit(
    ...
    def execute_swap(
        ...
        try:
            ...
            if dry_run:
                ...
                logging.debug(
                    "%s",
                    f"[DRY RUN] {amount/1e9:.6f} -> {out_amount} (impact: {price_impact:.2f}%)",
                )
                return result
            ...
            swap_resp = request_json(
                req, timeout=15, describe="Jupiter swap transaction"
            )
            ...
            if "result" in rpc_resp:
                ...
                logging.debug("%s", f"[EXECUTED] TX: {rpc_resp['result'][:20]}...")
            else:
                result["status"] = "failed"
                result["error"] = sanitize_sensitive_text(
                    rpc_resp.get("error", {}).get("message", "Unknown error")
                )
                logging.debug("%s", f"[FAILED] {result['error']}")

        except Exception as e:
            sanitized_error = sanitize_sensitive_text(e)
            result["status"] = (
                "blocked"
                if "Hard position limit exceeded" in sanitized_error
                else "error"
            )
            result["error"] = sanitized_error
            logging.debug("%s", f"[{result['status'].upper()}] {sanitized_error}")
