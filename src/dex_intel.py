                    ...
                    discrepancies.append(
                        {
                            ...
                            "avg_price_impact": sum(i["price_impact"] for i in items)
                            / len(items),
                        }
                    )
...
if __name__ == "__main__":
    intel = DexRouteIntel()
    logging.debug("%s", intel.summary())
    logging.debug("")

    # Find discrepancies for SOL/USDC
    disc = intel.find_route_discrepancies(SOL_MINT, USDC_MINT)
    if disc:
        logging.debug("%s", "Route discrepancies found:")
        for d in disc:
            logging.debug(
                "%s",
                f"  {d['route']}: sizes={d['sizes']}, avg_impact={d['avg_price_impact']:.4f}%",
            )
    else:
        logging.debug("%s", "No route discrepancies (single route used)")
